...
export default function SettingsPage() {
	...
	const handleToggleRepo = useCallback(
		async (repoId: string, currentStatus: boolean, repoName: string) => {
			...
			try {
				...
				if (!res.ok) {
					...
					toast.error("Failed to update repository", {
						description:
							"Could not change monitoring status. Please try again.",
					});
				} else {
					toast.success(
	...
	const repoUsagePercent =
		repoLimit === -1
			? 0
			: Math.min(100, (activeRepos.length / repoLimit) * 100);
	const isRepoLimitReached =
		repoLimit !== -1 && activeRepos.length >= repoLimit;
	...
	const githubInstallation = installations.find(
		(i) => i.providerType === "github" && i.status === "active",
	);
						...
						{loadingInstallations ? (
							...
							<div className="p-4 rounded-sm bg-secondary/30 border border-border/50 flex items-center gap-4">
								...
								<div className="flex-1 min-w-0">
									<div className="font-medium text-sm">GitHub</div>
									<div className="text-xs text-muted-foreground">
										{githubInstallation.accountLogin} - Connected
									</div>
								</div>
				...
				<section>
					<div className="flex items-center gap-3 mb-6">
						...
						<div className="flex-1">
							...
							<p className="text-sm text-muted-foreground">
								Choose which repositories Memoria actively monitors
							</p>
						</div>
						<div className="text-sm text-muted-foreground">
							{Object.values(repoStatuses).filter(Boolean).length} /{" "}
							{repoLimit === -1 ? "∞" : repoLimit} active
						</div>
					</div>
					...
					<div className="p-5 rounded-sm bg-secondary/30 border border-border/50">
						...
						<div className="mb-4 p-3 bg-primary/5 border border-primary/10 rounded-sm">
							<p className="text-sm text-muted-foreground">
								Active repositories receive real-time AI monitoring on pull
								requests. Inactive repos remain synced but won't receive
								automatic analysis.
							</p>
						</div>
								...
								<>
									...
									{githubInstallation && canAddRepo && (
										<div className="mt-4 pt-4 border-t border-border/50">
											<Button variant="outline" size="sm" asChild>
												<a
													href={`https://github.com/apps/memoria-dev/installations/${githubInstallation.providerInstallationId}`}
													target="_blank"
						...
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
							<div>
								<div className="flex items-center gap-2">
									...
									<span className="px-1.5 py-0.5 text-[10px] font-medium bg-primary/20 text-primary rounded-sm">
										CURRENT
									</span>
						...
						<div className="grid gap-4 sm:grid-cols-2">
							...
							<div className="space-y-2">
								...
								{isRepoLimitReached ? (
									...
									<div className="text-xs text-muted-foreground">
										{repoLimit - activeRepos.length} remaining
									</div>
								)}
							</div>
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between text-sm">
									...
									<span className="font-medium">
										{billingStatus?.activeReposCount ?? 0} /{" "}
										{billingStatus?.plan?.maxAnalysesPerMonth === -1
											? "Unlimited"
											: (billingStatus?.plan?.maxAnalysesPerMonth ?? 50)}
									</span>
								</div>
								<div className="h-2 rounded-sm bg-secondary/50 border border-border/50 overflow-hidden">
									<div
				...
				<section>
					<div className="flex items-center gap-3 mb-6">
						...
						<div>
							...
							<p className="text-sm text-muted-foreground">
								Connect your local AI tools to team guardrails and memories
							</p>
						</div>
					</div>
					...
					<div className="p-5 rounded-sm bg-secondary/30 border border-border/50 space-y-6">
						...
						<div className="p-3 bg-primary/5 border border-primary/10 rounded-sm">
							<p className="text-sm text-muted-foreground">
								Team tokens allow your local MCP server to sync guardrails and
								memories from your account. Add the token to your{" "}
								<code className="px-1 py-0.5 bg-secondary/50 rounded text-xs font-mono">
									.memoria.json
								</code>{" "}
								config file.
							</p>
							...
							<div className="p-4 bg-primary/10 border border-primary/30 rounded-sm">
								...
								<div className="mt-3 p-2 bg-background/50 border border-border/50 rounded-sm">
									<p className="text-xs text-muted-foreground mb-1">
										Add to your <code className="font-mono">.memoria.json</code>
										:
									</p>
						...
						<div>
							...
							<div className="space-y-3 text-xs text-muted-foreground">
								<div className="flex gap-2">
									<span className="w-5 h-5 rounded-full bg-primary/20 text-primary flex items-center justify-center shrink-0 text-[10px] font-medium">
										1
									</span>
								...
								<div className="flex gap-2">
									...
									<span>
										...
										<code className="px-1 py-0.5 bg-secondary/50 rounded font-mono">
											.memoria.json
										</code>{" "}
										file
									</span>
								...
								<div className="flex gap-2">
									...
									<span>
										Restart your AI tool (Cursor, Claude Code, etc.) to sync
										guardrails
									</span>
								</div>
							</div>
							<div className="mt-3 p-3 bg-background/50 border border-border/50 rounded-sm">
								<p className="text-xs text-muted-foreground mb-2">
									Example <code className="font-mono">.memoria.json</code>:
								</p>
