...
export function DashboardShell({ children }: { children: React.ReactNode }) {
	...
	const { user, activeRepos, billingStatus, canAddRepo, repoLimit, logout } =
		useDashboard();
	...
	const handleSync = useCallback(async () => {
		...
		const repoName = pathname
			.split("/dashboard/repositories/")[1]
			?.split("/")[0];
		const currentRepo = activeRepos.find(
		...
		try {
			const response = await fetch(
				`/api/repositories/${currentRepo._id}/scan`,
				{
					method: "POST",
				},
			);
								...
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										...
										>
											...
											<span
												className="px-1.5 py-0.5 text-xs bg-background/80 rounded-sm tabular-nums"
												aria-hidden="true"
											>
												{activeRepos.length}
											</span>
									...
									>
										...
										<div className="px-2.5 py-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground/70">
											Connected Repositories ({activeRepos.length}/
											{repoLimit === -1 ? "∞" : repoLimit})
										</div>
									...
									<div className="flex items-center gap-2">
										<DropdownMenu>
											...
											<DropdownMenuContent align="start" className="w-40">
												{(
													Object.entries(dateRangeLabels) as [
														DateRange,
														string,
													][]
												).map(([value, label]) => (
										...
										<DropdownMenu>
											...
											<DropdownMenuContent align="start" className="w-44">
												{(
													Object.entries(riskFilterLabels) as [
														RiskFilter,
														string,
													][]
												).map(([value, label]) => (
