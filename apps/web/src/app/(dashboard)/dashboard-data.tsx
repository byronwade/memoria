		...
		toCheck.map(async (inst) => {
			try {
				...
				if (newStatus !== inst.status) {
					...
					console.log(
						`[auto-refresh] Installation ${inst.providerInstallationId} status: ${inst.status} -> ${newStatus}`,
					);
				...
				if (isNotFound) {
					...
					console.log(
						`[auto-refresh] Installation ${inst.providerInstallationId} was deleted from GitHub`,
					);
				...
				console.error(
					`[auto-refresh] Failed to check installation ${inst.providerInstallationId}:`,
					error,
				);
				return inst;
			}
...
export async function getDashboardData(): Promise<DashboardData | null> {
	...
	const needsOnboarding =
		activeInstallations.length === 0 || activeRepos.length === 0;
