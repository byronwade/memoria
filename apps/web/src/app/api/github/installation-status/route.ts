...
export async function GET(_request: NextRequest) {
	try {
		...
		console.log(
			"[installation-status] Session token:",
		...
		console.log(
			"[installation-status] Session user:",
		...
		let installations =
			allInstallations?.filter((i) => i.status === "active") || [];

		console.log(
			"[installation-status] Installations in DB:",
		...
		if (installations.length === 0) {
			console.log(
				"[installation-status] No installations in DB, checking GitHub API...",
			);
			...
			if (identity?.accessToken) {
				try {
					...
					for (const inst of githubInstallations) {
						...
						console.log(
							"[installation-status] Created installation:",
							inst.id,
							inst.account.login,
						);
						...
						if (status === "active") {
							try {
								...
								console.error(
									"[installation-status] Failed to sync repos:",
									repoError,
								);
							}
						}
					...
					console.error(
						"[installation-status] Failed to query GitHub:",
						githubError,
					);
				}
			} else {
		...
		if (installations && installations.length > 0) {
			...
			console.log(
				"[installation-status] Repositories found:",
...
export async function POST(_request: NextRequest) {
	try {
		...
		for (const inst of installations) {
			console.log(
				`[resync] Syncing repos for installation ${inst.providerInstallationId}`,
			);
