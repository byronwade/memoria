...
/**
 * Dependency-free precision pass for the regex extractors: blank out comments
 * and string/template literals before matching, and recognize more export forms.
 * This removes the main false positives of regex extraction without a parser.
 */
describe("stripCommentsAndStrings", () => {
	...
	it("blanks string and template literals", () => {
		const src = "const s = \"export const inString\"; const t = `export const inTemplate`;";
		const out = stripCommentsAndStrings(src);
...
describe("extractExports precision", () => {
	...
	it("still extracts the classic forms (back-compat)", () => {
		...
		`;
		const exports = extractExports(code);
		for (const name of ["myFunction", "MY_CONST", "MyInterface", "foo", "bar", "defaultFunc"]) {
