...
import { cache, calculateHotspotScore, getVolatility } from "../src/index.js";
import { TempGitRepo } from "./helpers/temp-git-repo.js";

const DAY = 24 * 60 * 60 * 1000;

/**
 * Bugspots hotspot score (Google, 2011; formula from igrigorik/bugspots) and
 * Bird et al. (FSE 2011) minor-contributor ownership signal, both layered onto
...
describe("Volatility engine — bugspots hotspot + ownership", () => {
	describe("calculateHotspotScore (pure)", () => {
		...
		it("weights recent fixes far more than old ones", () => {
			...
			// Same count, same oldest anchor — the more recent second fix scores higher.
			expect(calculateHotspotScore(recent, now)).toBeGreaterThan(
				calculateHotspotScore(middle, now),
			);
		});

		it("increases as more recent fixes pile up (fix density)", () => {
			const now = Date.UTC(2026, 0, 1);
			const oldest = new Date(now - 100 * DAY);
	...
	describe("getVolatility integration", () => {
		...
		it("counts bug-fix commits and produces a positive hotspot score", async () => {
			// Spread, recent-leaning dates so the bugspots sigmoid has a real span.
			...
			await repo.commit("refactor foo", { "foo.ts": "v3" }, { date: iso(20) }); // weight 0.5 -> NOT a fix
			...
			// "fix crash", "fix bug", "hotfix regression" are fixes; "refactor" is not.
			expect(v.bugFixCommits).toBe(3);
			expect(v.hotspotScore).toBeGreaterThan(0);
		});

		it("flags many minor (<5% ownership) contributors (Bird et al.)", async () => {
			// 18 commits by the owner + 3 one-off authors = 21 total.
			// 1/21 ≈ 4.76% < 5% -> each one-off author is a 'minor' contributor.
			for (let i = 0; i < 18; i++) {
			...
			const v = await getVolatility(repo.path("foo.ts"));
			expect(v.minorContributors).toBe(3); // a, b, c — owner is the majority
		});
		...
		it("reports zero minor contributors when one author owns everything", async () => {
			...
			const v = await getVolatility(repo.path("foo.ts"));
			expect(v.minorContributors).toBe(0); // solo author owns 100%
		});
	});
});

/** Commit a set of files authored by a specific identity. */
async function commitAs(
	...
	await repo.git.raw(["commit", "-m", message, "--author", `${name} <${email}>`]);
}
